...
type Message struct {
	MessageId      bson.ObjectID  `json:"messageId" bson:"Id"`                              // 主键
	ConversationId bson.ObjectID  `json:"conversationId" bson:"conversationId"`             // 归属的对话id
	SectionId      bson.ObjectID  `json:"sectionId" bson:"sectionId"`                       // 归属的段落id
	UserId         bson.ObjectID  `json:"userId" bson:"userId"`                             // 用户id
	Index          int32          `json:"index" bson:"index"`                               // 消息索引
	ReplyId        bson.ObjectID  `json:"replyId,omitempty" bson:"replyId,omitempty"`       // 回复id, 只有模型消息有
	Content        string         `json:"content" bson:"content"`                           // 消息内容, json字符串
	ContentType    int32          `json:"contentType" bson:"contentType"`                   // 内容类型, text/think/suggest, 依次为0,1,2
	MessageType    int32          `json:"messageType" bson:"messageType"`                   // 消息类型, 默认为text, 0
	...
	DeleteTime     time.Time      `json:"deleteTime,omitempty" bson:"deleteTime,omitempty"` // 删除时间
